...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Animate electron or ion density profiles from 35x35, 70x70, and 140x140 grids. "
    ...
    for grid in GRID_NAMES:
        parser.add_argument(
            f"--dir-{grid}",
            required=True,
            help=f"Directory containing rho_e or rho_i dumps for the {grid}x{grid} grid.",
        )
    parser.add_argument(
        "--species",
        choices=("electron", "ion"),
        required=True,
        help="Particle species to compare; selects rho_e or rho_i files.",
    )
    parser.add_argument(
        "--pattern",
        default=None,
        help="Optional glob override. Defaults to rho_e_[0-9]*.txt or rho_i_[0-9]*.txt.",
    )
    parser.add_argument(
        "--quantity",
        choices=("number-density", "charge-density"),
        default="number-density",
        help="Plot number density or the raw signed charge density. Default: %(default)s",
    )
    parser.add_argument(
        "--output",
        default="density_profile_multigrid_comparison.mp4",
...
def convert_density(frame: np.ndarray, species: str, quantity: str) -> np.ndarray:
    if quantity == "charge-density":
        return frame
    sign = -1.0 if species == "electron" else 1.0
    return sign * frame / ELEMENTARY_CHARGE
...
def main() -> None:
    ...
    species_symbol = "e" if args.species == "electron" else "i"
    pattern = args.pattern or f"rho_{species_symbol}_[0-9]*.txt"
    frame_maps = {label: load_frame_map(directory, pattern) for label, directory in directories.items()}
    frame_maps = {
        label: {step: convert_density(frame, args.species, args.quantity) for step, frame in frames.items()}
        for label, frames in frame_maps.items()
    }
    for label, frames in frame_maps.items():
        if not frames:
            raise SystemExit(f"No valid frames matched {pattern} in {directories[label]} ({label}).")
    ...
    density_symbol = r"n_e" if args.species == "electron" else r"n_i"
    charge_symbol = r"\rho_e" if args.species == "electron" else r"\rho_i"
    if args.quantity == "number-density":
        ax.set_ylabel(rf"${density_symbol}$ (m$^{{-2}}$)", fontsize=15)
    else:
        ax.set_ylabel(rf"${charge_symbol}$ (C m$^{{-2}}$)", fontsize=15)
    ax.set_xlim(-1.0, 1.0)
    ...
    def update(frame_index: int):
        ...
        stat_text.set_text(
            f"RMS(35−70) = {np.sqrt(np.mean(diff_35 * diff_35)):.3e}\n"
            f"RMS(140−70) = {np.sqrt(np.mean(diff_140 * diff_140)):.3e}"
        )
