...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Animate and compare ion-density line profiles from two folders."
    )
    parser.add_argument(
        "--reference-dir",
        required=True,
        help="Directory with Penning reference rho_i files, e.g. rho_i_840000.txt",
    )
    parser.add_argument(
        "--jax-dir",
        required=True,
        help="Directory with JAX rho_i files, e.g. rho_i_840000.txt",
    )
    parser.add_argument(
        "--reference-pattern",
        default="rho_i_[0-9]*.txt",
    ...
    parser.add_argument(
        "--output",
        default="rho_i_profile_comparison.mp4",
...
def main() -> None:
    ...
    ax.set_title("Ion Density Profile Comparison", fontsize=22, pad=14)
    ax.set_xlabel(x_label, fontsize=15)
    ax.set_ylabel(r"$n_i$", fontsize=15)
    ...
    def update(frame_idx: int):
        ...
        stat_text.set_text(
            f"max |dn_i| = {np.max(np.abs(diff)):.3e}\n"
            f"mean dn_i = {np.mean(diff):.3e}\n"
            f"RMS dn_i = {np.sqrt(np.mean(diff * diff)):.3e}"
        )
