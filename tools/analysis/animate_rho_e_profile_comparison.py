...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Animate and compare electron-concentration line profiles from two folders."
    )
    parser.add_argument(
        "--reference-dir",
        required=True,
        help="Directory with Penning reference rho_e files, e.g. rho_e_840000.txt",
    )
    parser.add_argument(
        "--jax-dir",
        required=True,
        help="Directory with JAX rho_e files, e.g. rho_e_840000.txt",
    )
    parser.add_argument(
        "--reference-pattern",
        default="rho_e_[0-9]*.txt",
    ...
    parser.add_argument(
        "--output",
        default="rho_e_profile_comparison.mp4",
...
def main() -> None:
    ...
    if not reference_frames:
        raise SystemExit(
            f"No valid Penning reference frames matched {args.reference_pattern} in {reference_dir}"
        )
    if not jax_frames:
    ...
    if not common_steps:
        raise SystemExit(
            "No common iteration numbers were found between Penning reference and JAX folders."
        )
    ...
    reference_profiles = [
        extract_profile(reference_frames[step], args.profile_axis, fixed_index)
        for step in common_steps
    ]
    jax_profiles = [
        extract_profile(jax_frames[step], args.profile_axis, fixed_index)
        for step in common_steps
    ]
    ...
    (jax_line,) = ax.plot(
        x_values,
        jax_profiles[0],
        color=args.jax_color,
        lw=2.6,
        label=args.jax_label,
    )

    ax.set_title("Electron Concentration Profile Comparison", fontsize=22, pad=14)
    ax.set_xlabel(x_label, fontsize=15)
    ax.set_ylabel(r"$n_e$", fontsize=15)
    ...
    def update(frame_idx: int):
        ...
        stat_text.set_text(
            f"max |dn_e| = {np.max(np.abs(diff)):.3e}\n"
            f"mean dn_e = {np.mean(diff):.3e}\n"
            f"RMS dn_e = {np.sqrt(np.mean(diff * diff)):.3e}"
        )
